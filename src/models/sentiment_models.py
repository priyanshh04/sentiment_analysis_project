...
# Utility functions
def quick_train_sentiment_model(df: pd.DataFrame, 
                              text_column: str = 'reviewText',
                              target_column: str = 'sentiment_binary') -> SentimentModelTrainer:
    """
    Quick training function for sentiment analysis
    ...
    Returns:
        SentimentModelTrainer: Trained model trainer
    """
    trainer = SentimentModelTrainer()
    trainer.prepare_data(df, text_column, target_column)
    trainer.train_all_models()
    return trainer

def compare_models(results: Dict[str, Any]) -> None:
    """
    ...
    for model_name, result in results.items():
        ...
        print(f"  Test F1-Score: {result.get('test_f1', 0):.4f}")
        print(f"  Test ROC-AUC:  {result.get('test_roc_auc', 0):.4f}")
        if 'cv_mean' in result:
            print(f"  CV F1 Mean:    {result['cv_mean']:.4f} (±{result['cv_std']:.4f})")
